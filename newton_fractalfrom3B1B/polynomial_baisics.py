...
class ConstructPolynomialWithGivenRoots(Scene):
    ...
    def construct(self):
        ...
        # Reverse question
        top_lhs = MathTex("P(x)").match_height(factored)
        ...
        self.wait()
        root_eqs = VGroup(*(
            VGroup(MathTex(f"r_{i} ", "="), DecimalNumber(root, num_decimal_places=3)).arrange(RIGHT)
...
class FactsAboutRootsToCoefficients(RootCoefScene):
    ...
    def construct(self):
        ...
        arrow = MathTex("\\Downarrow", font_size=36)
    ...
    def add_solver_functions(self):
        ...
        lines = VGroup(*(
            MathTex(f"r_{i} = {func_name}_{i}(c_0, c_1, c_2)", **kw)
            for i in range(3)
        ))
        lines.scale(0.8)
...
class SolvabilityChart(Scene):
    def construct(self):
        ...
        solvable_word = MathTex("Can you solve\\\\for $x$?")
        ...
        lin_solution = MathTex("x = {-{b} \\over {a}}", **tex_kw)
        ...
        # Cubic
        key_to_color = dict([
            (TransformMatchingShapes.get_mobject_key(MathTex(c)[0][0]), color)
    ...
    def get_equations(self, words):
        ...
        equations = VGroup(
            MathTex("{a}x + {b} = 0", **kw),
            MathTex("{a}x^2 + {b}x + {c} = 0", **kw),
            MathTex("{a}x^3 + {b}x^2 + {c}x + {d} = 0", **kw),
            MathTex("{a}x^4 + \\cdots + {d}x + {e} = 0", **kw),
            MathTex("{a}x^5 + \\cdots + {e}x + {f} = 0", **kw),
            MathTex("{a}x^6 + \\cdots + {f}x + {g} = 0", **kw),
            MathTex("\\vdots", **kw),
        )
    ...
    def get_solvability_words(self, equations):
        ...
        s_words = VGroup(
            MathTex("Solvable", " using\\\\", arith),
            MathTex("Solvable", " using\\\\", radicals),
            MathTex("Solvable\\\\", "numerically"),
        )
    ...
    def get_axes(self, frame):
        ...
        axes.add(MathTex("x", font_size=24).next_to(axes.x_axis.get_right(), DOWN, SMALL_BUFF))
        axes.add(MathTex("y", font_size=24).next_to(axes.y_axis.get_top(), LEFT, SMALL_BUFF))
        return axes
...
class StudySqrt(RadicalScene):
    ...
    def construct(self):
        ...
        c_label = VGroup(
            MathTex("c = ", tex_to_color_map={"c": self.coef_color}),
        ...
        r_labels = VGroup(*(
            VGroup(MathTex(f"r_{i}", "="), DecimalNumber(self.c, include_sign=True))
            for i in range(2)
        ))
        ...
        option = VGroup(
            MathTex("One option:", color=BLUE),
            MathTex("\\\\Make", " $\\sqrt{\\quad}$", " single-valued, but discontinuous", font_size=36),
        )
        ...
        f_label = MathTex("f_4(x)", font_size=24, color=GREEN)
    ...
    def get_exp_label(self, get_theta, color=GREEN):
        result = MathTex("e^{", "2\\pi i \\cdot", "0.00}")
...
class CubeRootBehavior(StudySqrt):
    ...
    def construct(self):
        ...
        c_label = MathTex("c = ", "1.00", tex_to_color_map={"c": self.coef_color})
        r_label = MathTex("r_0 = ", "1.00", tex_to_color_map={"r_0": self.root_color})
