...
# 关于着色器newton_fractal_shader
  ...
  然后我们声明了一个二维数组变量found_root，其储存了后面那个函数的结果，那个结果是什么呢？
    ...
    - `log(curr_len)` 表示当前步长向量 `curr_len` 的自然对数。
      ...
      在 Julia 集合渲染中，迭代次数越多，图像越精细。但是，迭代次数越多，计算量也越大。因此，这行代码可以用于调整迭代次数，以达到图像质量和计算量之间的平衡。

      在您提供的代码中，`threshold` 是一个阈值，用于判断迭代是否结束。如果当前步长向量 `curr_len` 小于 `threshold`，那么迭代就结束了。

      因此，这行代码可以用于调整迭代次数，以使迭代在 `threshold` 之前结束。这可以减少计算量，提高渲染速度。（并不能哦，跳出循环不归这个管的）
...
# 关于自定义Mobject与VMobject类型变量
- 1 我们通过筛选出来的点的坐标的话，我们该怎么办
- 2 我们能找出点围成的边界的内部的点，并且对其进行上色吗？
- 3 我们能找出外部的点，并且根据迭代次数进行上色吗？
